...
function useBluetoothPrinter() {
	try {
        log('Mencari printer Bluetooth...');
        
        // 1. Scan perangkat dengan Service UUID opsional yang kompatibel
        ...
        log(`Ditemukan: ${bluetoothDevice.name || 'Printer Bluetooth'}`);
        log('Menghubungkan...');

        const server = await bluetoothDevice.gatt.connect();
        log('Connected to GATT Server');

        // 2. Ambil SEMUA primary service yang ada di printer (tanpa filter UUID spesifik)
        const services = await server.getPrimaryServices();
        if (services.length === 0) {
            throw new Error("Tidak ada Service Bluetooth yang ditemukan pada perangkat ini.");
        }
        log(`Ditemukan ${services.length} Service`);

        // 3. Cari Characteristic untuk Write di service yang tersedia
        ...
        for (const service of services) {
            try {
                ...
                if (foundChar) {
                    characteristicWrite = foundChar;
                    log(`Write characteristic ditemukan pada service: ${service.uuid}`);
                    break;
                }
            } catch (err) {
                // Lanjut ke service berikutnya jika service ini gagal diakses
                continue;
            }
        }

        if (!characteristicWrite) {
            throw new Error("Saluran penulisan data (Write Characteristic) tidak ditemukan.");
        }

        log('✓ Printer berhasil terhubung!');
        updateStatus(true);
...
async function printRawWebBluetooth(rawText, onStatus) {
  if (!navigator.bluetooth) {
    throw new Error("Web Bluetooth tidak didukung. Gunakan Chrome Android 85+");
  }
...
// ── Web Bluetooth GATT Thermal Printer (EPPOS RPP02) ─────────────────────────
// Protokol: GATT over Bluetooth LE/Classic SPP
// UUID SPP standar thermal printer China
...
const ESC_BOLD_ON     = [0x1B, 0x45, 0x01];     // ESC E 1 - bold on
const ESC_BOLD_OFF    = [0x1B, 0x45, 0x00];     // ESC E 0 - bold off
const ESC_DOUBLE_ON   = [0x1B, 0x21, 0x30];     // ESC ! 48 - double height+width
const ESC_DOUBLE_OFF  = [0x1B, 0x21, 0x00];     // ESC ! 0 - normal size
const GS_CUT          = [0x1D, 0x56, 0x41, 0x00]; // GS V A 0 - cut paper
...
function buildEscPosPayload(r, settings) {
  const W = 32;
  const fRp = n => "Rp" + new Intl.NumberFormat("id-ID").format(n);
  const center = t => {
  ...
  const lr = (l, r) => {
    ...
    return l + " ".repeat(sp) + r.slice(0, W - l.length - 1);
  };
...
";
  ...
  // 1. Init printer
  chunks.push(new Uint8Array(ESC_INIT));

  // 2. Header - nama toko rata tengah, bold besar
  chunks.push(new Uint8Array(ESC_CENTER));
  chunks.push(new Uint8Array(ESC_DOUBLE_ON));
  chunks.push(new Uint8Array(ESC_BOLD_ON));
  chunks.push(strToBytes(settings.storeName.slice(0,16) + "
"));
  chunks.push(new Uint8Array(ESC_DOUBLE_OFF));
  chunks.push(new Uint8Array(ESC_BOLD_OFF));
  chunks.push(strToBytes(settings.address.slice(0,32) + "
"));
  chunks.push(new Uint8Array(ESC_LEFT));

  // 3. Info transaksi
  chunks.push(strToBytes(divider()));
  chunks.push(strToBytes("No: " + r.id.slice(-10) + "
"));
  chunks.push(strToBytes("Tgl: " + new Date(r.date).toLocaleString("id-ID",{dateStyle:"short",timeStyle:"short"}) + "
"));
  chunks.push(strToBytes("Kasir: " + (r.cashier || settings.cashier) + "
"));
  chunks.push(strToBytes("Bayar: " + r.payment.toUpperCase() + "
"));
  chunks.push(strToBytes(divider()));

  // 4. Item-item
  r.items.forEach(it => {
    const name = it.name.slice(0, W);
    chunks.push(new Uint8Array(ESC_BOLD_ON));
    chunks.push(strToBytes(name + "
"));
    chunks.push(new Uint8Array(ESC_BOLD_OFF));
    const qtyStr = "  " + it.qty + "x " + fRp(it.price);
    const subStr = fRp(it.qty * it.price);
    chunks.push(strToBytes(lr(qtyStr, subStr) + "
"));
  });

  // 5. Total
  chunks.push(strToBytes(divider()));
  chunks.push(strToBytes(lr("Subtotal", fRp(r.subtotal)) + "
"));
  if (r.discount > 0)
    chunks.push(strToBytes(lr("Diskon", "-" + fRp(r.discount)) + "
"));
  if (r.tax > 0)
    chunks.push(strToBytes(lr("Pajak", fRp(r.tax)) + "
"));
  chunks.push(strToBytes(divider("=")));
  chunks.push(new Uint8Array(ESC_BOLD_ON));
  chunks.push(new Uint8Array(ESC_DOUBLE_ON));
  chunks.push(strToBytes(lr("TOTAL", fRp(r.total)) + "
"));
  chunks.push(new Uint8Array(ESC_DOUBLE_OFF));
  chunks.push(new Uint8Array(ESC_BOLD_OFF));
  chunks.push(strToBytes(lr("Bayar(" + r.payment + ")", fRp(r.cashPaid)) + "
"));
  if (r.change > 0)
    chunks.push(strToBytes(lr("Kembalian", fRp(r.change)) + "
"));

  // 6. Catatan
  if (r.note) {
    chunks.push(strToBytes(divider()));
    chunks.push(strToBytes("Catatan: " + r.note + "
"));
  }
  ...
  chunks.push(strToBytes(divider()));
  chunks.push(new Uint8Array(ESC_CENTER));
  chunks.push(strToBytes("Terima kasih!
"));
  chunks.push(strToBytes("Kasir-Pro
"));
  chunks.push(new Uint8Array(ESC_LEFT));
...
"));
  chunks.push(new Uint8Array(GS_CUT));

  return mergeBytes(...chunks);
}

async function printGATT(r, settings, onStatus) {
  // TAHAP 1: Pemindaian & Otorisasi
  ...
  try {
    device = await navigator.bluetooth.requestDevice({
      // Tampilkan SEMUA perangkat Bluetooth agar RPP02 pasti muncul
      acceptAllDevices: true,
      optionalServices: PRINTER_SERVICE_UUIDS,
    });
  } catch (e) {
    if (e.name === "NotFoundError") throw new Error("Tidak ada printer dipilih");
    throw new Error("Gagal scan: " + e.message);
  }

  onStatus("🔗 Menghubungkan ke " + device.name + "...");

  // TAHAP 2: Koneksi GATT
  let server;
  try {
    server = await device.gatt.connect();
  } catch (e) {
    throw new Error("Gagal konek GATT: " + e.message);
  }

  onStatus("📡 Mencari jalur data (characteristic)...");

  // Cari write characteristic dari semua service
  let writeChar = null;
  const services = await server.getPrimaryServices().catch(() => []);

  // Coba satu per satu UUID service
  for (const svcUUID of PRINTER_SERVICE_UUIDS) {
    if (writeChar) break;
    try {
      const svc = await server.getPrimaryService(svcUUID);
  ...
  // Fallback: scan semua service yang tersedia
  if (!writeChar && services.length > 0) {
    for (const svc of services) {
      if (writeChar) break;
      try {
        const chars = await svc.getCharacteristics();
  ...
  if (!writeChar) {
    server.disconnect();
    throw new Error("Characteristic write tidak ditemukan. Pastikan pilih printer yang benar.");
  }

  // TAHAP 3: Build ESC/POS payload
  onStatus("📄 Menyusun data cetak ESC/POS...");
  const payload = buildEscPosPayload(r, settings);

  // TAHAP 4: Transmisi data dalam chunk 512 bytes
  ...
  try {
    for (let i = 0; i < payload.length; i += CHUNK_SIZE) {
      ...
      // Jeda kecil antar chunk agar buffer printer tidak overflow
      await new Promise(res => setTimeout(res, 60));
    }
  } catch (e) {
    server.disconnect();
    throw new Error("Gagal kirim data: " + e.message);
  }

  onStatus("✅ Cetak berhasil!");
...
// Fallback: buka HTML window untuk cetak via browser
function buildReceiptHTML(r, settings) {
  const fRp = n => "Rp " + new Intl.NumberFormat("id-ID").format(n);
  const rows = r.items.map(it =>
    `<tr><td>${it.name}</td><td style="text-align:center">${it.qty}x</td><td style="text-align:right">${fRp(it.price)}</td><td style="text-align:right">${fRp(it.qty*it.price)}</td></tr>`
...
<h2>${settings.storeName}</h2><div class="sub">${settings.address}</div>
<hr class="divider"/>
<p>No: ${r.id.slice(-10)}</p><p>Tgl: ${new Date(r.date).toLocaleString("id-ID",{dateStyle:"short",timeStyle:"short"})}</p>
<p>Kasir: ${r.cashier||settings.cashier}</p>
<hr class="divider"/>
<table><tbody>${rows}</tbody></table>
<hr class="divider"/>
...
// RPP02 pakai Bluetooth Classic (SPP) - tidak bisa dari Web Browser langsung
// Solusi: Generate HTML struk lalu buka Android Print Dialog
...
function buildReceiptHTML(r, settings) {
  ...
  const rows = r.items.map(it =>
    `<tr>
      ...
      <td style="text-align:right">${fRp(it.price)}</td>
      <td style="text-align:right">${fRp(it.qty * it.price)}</td>
    </tr>`
  ).join("");
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  table { width:100%; border-collapse:collapse; margin:2px 0; }
  td { padding:1px 1px; vertical-align:top; font-size:10px; }
  .grand td { font-weight:bold; font-size:12px; border-top:1px dashed #000; padding-top:3px; }
...
<body>
  ...
  <hr class="divider"/>
  <div class="info">No: ${r.id.slice(-10)}</div>
  <div class="info">Tgl: ${new Date(r.date).toLocaleString("id-ID",{dateStyle:"short",timeStyle:"short"})}</div>
  <div class="info">Kasir: ${r.cashier || settings.cashier}</div>
  ${r.payment ? `<div class="info">Bayar: ${r.payment.toUpperCase()}</div>` : ""}
  <hr class="divider"/>
  <table>
    <tbody>${rows}</tbody>
  </table>
  ...
  <table>
    <tbody>
      <tr class="total-row"><td>Subtotal</td><td style="text-align:right">${fRp(r.subtotal)}</td></tr>
      ${r.discount > 0 ? `<tr class="total-row"><td>Diskon</td><td style="text-align:right">-${fRp(r.discount)}</td></tr>` : ""}
      ${r.tax > 0 ? `<tr class="total-row"><td>Pajak</td><td style="text-align:right">${fRp(r.tax)}</td></tr>` : ""}
      <tr class="grand"><td>TOTAL</td><td style="text-align:right">${fRp(r.total)}</td></tr>
      <tr class="total-row"><td>Bayar</td><td style="text-align:right">${fRp(r.cashPaid)}</td></tr>
      ${r.change > 0 ? `<tr class="total-row"><td>Kembalian</td><td style="text-align:right">${fRp(r.change)}</td></tr>` : ""}
    </tbody>
  </table>
  ${r.note ? `<hr class="divider"/><div class="info">Catatan: ${r.note}</div>` : ""}
  <hr class="divider"/>
  <div class="footer">
    <div>Terima kasih!</div>
    <div style="margin-top:2px;font-size:9px">Kasir-Pro</div>
  </div>
  <br/><br/>
  <script>
...
function buildPlainText(r, settings) {
  // Plain text ESC/POS untuk Quick Printer / intent
  const W = 32;
  ...
  let txt = "";
  txt += center(settings.storeName) + "\n";
  txt += center(settings.address) + "\n";
  txt += line() + "\n";
  txt += "No: " + r.id.slice(-10) + "\n";
  txt += "Tgl: " + new Date(r.date).toLocaleString("id-ID",{dateStyle:"short",timeStyle:"short"}) + "\n";
  txt += "Kasir: " + (r.cashier||settings.cashier) + "\n";
  txt += line() + "\n";
  r.items.forEach(it => {
    const nm = it.name.length>20 ? it.name.slice(0,20) : it.name;
    txt += nm + "\n";
    txt += lr("  "+it.qty+"x "+fRp(it.price), fRp(it.qty*it.price)) + "\n";
  });
  txt += line() + "\n";
  txt += lr("Subtotal", fRp(r.subtotal)) + "\n";
  if(r.discount>0) txt += lr("Diskon", "-"+fRp(r.discount)) + "\n";
  if(r.tax>0) txt += lr("Pajak", fRp(r.tax)) + "\n";
  txt += line("=") + "\n";
  txt += lr("TOTAL", fRp(r.total)) + "\n";
  txt += lr("Bayar("+r.payment+")", fRp(r.cashPaid)) + "\n";
  if(r.change>0) txt += lr("Kembalian", fRp(r.change)) + "\n";
  if(r.note) { txt += line() + "\n"; txt += "Catatan: "+r.note + "\n"; }
  txt += line() + "\n";
...
function printViaQuickPrinter(r, settings) {
  // Kirim ke Quick Printer via deep link (Android only)
  const text = buildPlainText(r, settings);
  const encoded = encodeURIComponent(text);
  // Quick Printer deep link
  const quickPrinterURL = "quickprinter://print?text=" + encoded;
  window.location.href = quickPrinterURL;
  return true;
}

function printViaRawBT(r, settings) {
  // RawBT app deep link - salah satu app paling populer untuk thermal Bluetooth Classic
  const text = buildPlainText(r, settings);
        ...
        <main className="content">
          ...
          {page==="dashboard" && (
            <div className="page-content">
              ...
              <div className="m3-card"><h4>Transaksi Terbaru</h4>{transactions.slice(0,6).map(t=>(<div key={t.id} className="list-row"><span className="lr-id">{t.id}</span><span className="lr-badge">{t.payment}</span><span className="lr-total">{formatRp(t.total)}</span></div>))}</div>
          ...
          {page==="transactions" && (
            <div className="page-content">
              <div className="page-toolbar">
                <div className="page-title" style={{marginBottom:0,flex:1}}>Transaksi</div>
